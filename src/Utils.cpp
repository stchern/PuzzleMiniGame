...
std::vector<std::vector<Position>> Utils::adjacencyMatrix(const Matrix& matrix, const std::vector<Sequence>& sequences)
{
    ...
    for (size_t seqIdx = 0; seqIdx < sequences.size(); ++seqIdx)
        for (size_t row = 0; row < matrix.rowCount(); ++row) {
            ...
            if (isFoundInRow(matrix, Position(row, -1), sequences[seqIdx].codes()[0], startPositionsForSequence))
        ...
        size_t maxLengthPath)
{
    ...
    for (size_t seqIdx = 0; seqIdx < adjacencyMatrix.size(); ++seqIdx)
        for (size_t currStartPosIdx = 0; currStartPosIdx < adjacencyMatrix[seqIdx].size(); ++currStartPosIdx) {
            std::vector<Path> purePathForCurr = DFS(matrix, sequences[seqIdx], adjacencyMatrix[seqIdx][currStartPosIdx], maxLengthPath);
        ...
        size_t maxLengthPath)
{
    ...
    for (size_t currSeqIdx = 0; currSeqIdx < purePaths.size(); ++currSeqIdx) {
        ...
        for (size_t pathIdx = 0; pathIdx < purePaths[currSeqIdx].size(); ++pathIdx) {
            Path currPath;
            if (isPossibleAddWastedMovesBeforeFirstSequences(purePaths[currSeqIdx][pathIdx], matrix.columnCount(), maxLengthPath, currPath)) {
                std::vector<std::pair<Path, int>> possiblePathsForCurrent = combinePurePath(
                            purePaths, currPath,
                            currSequence,
...
std::vector<Path> Utils::Internal::DFS(
        const Matrix& matrix,
...
std::vector<Path> Utils::Internal::BFS(
        const Matrix& matrix,
...
bool Utils::Internal::isFoundNextPositionsInColsAndRowsByPosition(
        const Matrix& matrix,
...
bool Utils::Internal::isValid(const Position& currPosition, const Matrix& isVisitedMatrix, const Matrix& matrix, unsigned char seqValue)
{
    if (isVisitedMatrix.value(currPosition))
        return false;
    return true;
...
bool Utils::Internal::isFoundNextPositionsInColsAndRowsByValue(
        const Matrix& matrix,
        const std::pair<Position, Path>& startPositionAndPath,
        unsigned char value,
        std::stack<std::pair<Position, Path>>& stack,
...
bool Utils::Internal::isFoundInRow(const Matrix& matrix,  const Position& startPosition, unsigned char value, std::vector<Position>& outVector)
{
...
bool Utils::Internal::isFoundInCol(const Matrix& matrix,  const Position& startPosition, unsigned char value, std::vector<Position>& outVector)
{
...
Matrix Utils::Internal::createIsVisitedMatrix(int row, int col, const Position& startPosition)
{
...
void Utils::Internal::setVisitedPositions(Matrix& isVisitedMatrix, const Path& visitedPath)
{
...
std::vector<std::pair<Path, int>> Utils::Internal::combinePurePath(
        const std::vector<std::vector<Path>>& purePaths,
        const Path& currPath,
...
bool Utils::Internal::isCreatedPathsAfterCurrent(
        const Path& currPath,
        const Path& nextPath,
        const Sequence& currSequence,
...
bool Utils::Internal::hasOverlapping(const Sequence& rhsSeq, const Sequence& lhsSeq, size_t& overlapLength)
{
...
bool Utils::Internal::hasIntersection(const Path& rhsPath, const Path& lhsPath)
{
...
bool Utils::Internal::isPossibleAddWastedMovesBetweenSequences(const Path& rhsPath, const Path& lhsPath, const Matrix& matrix, size_t maxLengthPath, std::vector<Path>& outPaths)
{
    ...
    if (sizeFirst > 1 && sizeSecond > 1) {
        ...
        if (isFinishByColFirst || !isStartByColSecond) {
            ...
            intermediatePaths = Utils::Internal::BFS(matrix, lastPosOfPathFirst, firstPosOfPathSecond, isStartByColSecond, !isFinishByColFirst);
    ...
    else
    {
        ...
        {
            bool isStartByColSecond = !(firstPosOfPathSecond.column() - lhsPath.positions()[1].column());
            possibleIntermediatePaths.push_back(Utils::Internal::BFS(matrix, lastPosOfPathFirst, firstPosOfPathSecond, isStartByColSecond, true));
            possibleIntermediatePaths.push_back(Utils::Internal::BFS(matrix, lastPosOfPathFirst, firstPosOfPathSecond, isStartByColSecond, false));
        }
        else
        {
            bool isFinishByColFirst = !(lastPosOfPathFirst.column() - rhsPath.positions()[sizeFirst - 2].column());
            possibleIntermediatePaths.push_back(Utils::Internal::BFS(matrix, lastPosOfPathFirst, firstPosOfPathSecond, true, !isFinishByColFirst));
            possibleIntermediatePaths.push_back(Utils::Internal::BFS(matrix, lastPosOfPathFirst, firstPosOfPathSecond, false, !isFinishByColFirst));
        }
...
bool Utils::Internal::isPossibleAddWastedMovesBeforeFirstSequences(const Path& path, int maxColumnCount, size_t maxLengthPath, Path& outPath)
{
...
bool Utils::Internal::addOneWastedMoveBeforeFirstSequence(const Path& path, Path& outPath)
{
    Position firstPathPosition = path.positions()[0];
...
bool Utils::Internal::addTwoWastedMovesBeforeFirstSequence(const Path& path, int maxColumnCount, Path& outPath)
{
    Position firstPathPosition = path.positions()[0];
...
bool Utils::Internal::isFoundNotVisitedPositionInRow(const std::vector<Position>& row, const Position& currPosition, const std::vector<bool>& isVisitedCol, int maxColumnCount, int& notVisitedCol)
{
...
bool Utils::Internal::isPositionIsAnyPaths(const Path& rhsPath, const Path& lhsPath, const Position& position)
{
...
Path Utils::Internal::concatenatePaths(const Path& rhsPath, const Path& lhsPath)
{
    Path resultPath;
